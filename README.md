...
> **口径**：期刊/会议名、年份、DOI、卷期与中科院分区均经 Crossref、PubMed 或出版社官网核实；⚠️ 条目含更正/待核实提示时以提示为准，正式引用前请以 DOI 复核。
> **维护**：直接在 GitHub 上打开本文件 → ✏️ 编辑 → 修改表格行 → *Commit changes*，主页即时更新（无需本地安装 git）。

## 📌 状态图例

| 标记 | 含义 |
|---|---|
| ✅ | **计划引用**（原清单 A 档/必引或一区正式版核心条目） |
| 📥 | **候选补充**（B/C 档、跨部位（脑/脊柱/MRI）或权威性较低条目） |
| 🔍 | **预印本追踪**（尚未正式发表，写稿前追新用） |

> 状态分类沿用本地调研清单的 A/B/C 档位口径整理而成，可按你的实际引用计划直接在表中修改。
...
| **RadCLIP** | IEEE TNNLS（一区 TOP） | 2025（10 月卷期） | 📥 | DOI: [10.1109/tnnls.2025.3568036](https://doi.org/10.1109/tnnls.2025.3568036)；⚠️ 期刊版为 **2025-10**，非 2024（arXiv 2024-03 先行） |
...
| [**Med-Flamingo**](https://arxiv.org/abs/2307.15189) | ML4H@NeurIPS 2023（workshop） | 2023 | 🔍 | ⚠️ 常被引为 npj Digital Medicine 2024 的期刊版**未能核实**，引用前请自行确认 |
...
- **新增一篇**：在对应分类表末尾复制一行 → 填「论文名（英文全称或简称）」→ 把标题/链接贴上 → 填「期刊/会议 + 年份」→ 状态先标 🔍 → 点 *Preview* 确认表格不错位 → *Commit changes*。
- **字段口径**：期刊/会议列建议带**中科院分区简注**（参考口径见文末）；备注列写一句话要点与 ⚠️ 更正提示；每个单元格不要出现裸的 `|` 字符（会破坏表格）。
- **状态调整**：✅ → 📥 → 🔍 按实际引用计划直接改。
...
2. **易错条目**：CT-GEN 期刊正式版 **2026-02**（勿误标 2025）；RadCLIP 期刊版 **TNNLS 2025-10**（勿标 2024）；Abn-BLIP、S-LMR+CSE 为 2025 注册 DOI、2026 卷期。
3. 会议与期刊并存条目（M3D、RadFM、Enhancing 3D…）：正式期刊版优先引用，会议版作补充。
4. 所有 DOI/链接均为整理时实测可访问；卷期页码等细节请在正式引用前用 DOI 复核。

## 📅 更新记录

- **2026-09-04**：初始收录。自本地调研清单去重整理：6 类正式条目（数据集 4 / 期刊 10 / 会议 4 / 通用模型 9 / 跨部位 6 / 综述 4）+ 预印本追踪 13；状态按原清单 A/B/C 档位与"必引"标注映射。
